...
function wayback(){
    ...
    waybackurls $domain | grep -a -v '\.\(jpg\|jpeg\|gif\|css\|tiff\|png\|ttf\|woff\|woff2\|ico\|pdf\|svg\)$' | sort -u >> subdomains/$domain/gau/gau_results.txt

    gau $domain | grep -a -v '\.\(jpg\|jpeg\|gif\|css\|tiff\|png\|ttf\|woff\|woff2\|ico\|pdf\|svg\)$' | sort -u >> subdomains/$domain/gau/gau_results.txt
...
# while getopts p:w:j:c:a: options; do
#     case $options in
#         p) params "$OPTARG";;    # execute params fucntion
